...
import { RpcErrorPayload } from '@/interface';
import { RMQ_SERVICE } from '@/rabbitmq/rabbitmq.token'; 

@Catch()
export class MsExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(MsExceptionFilter.name);

  private hasRpcErrorPayload(exception: unknown): exception is { error: RpcErrorPayload } {
    return (
    ...
    try {
      ...
      if (
        ...
        pattern = typeof patternResult === 'string' ? patternResult : String(patternResult);
      }
    } catch (error) {
      throwError(() => error);
    }
    ...
    if (exception instanceof HttpException) {
      ...
      return throwError(() => {
        ...
        if (typeof response === 'string') {
          ...
          if ('errors' in response || 'details' in response || 'exceptionType' in response) {
            details = response;
          }
        } else {
          message = exception.message;
